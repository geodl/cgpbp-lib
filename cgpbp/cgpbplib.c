...
struct parameters {
	...
	void (*mutationType)(struct parameters *params, struct chromosome *chromo, unsigned int * seed);
	...
	void (*reproductionScheme)(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren, unsigned int * seed);
	char reproductionSchemeName[REPRODUCTIONSCHEMENAMELENGTH];
	int numThreads;

	// BP parameter
	unsigned int bp_max_epochs;
};
...
/* mutation functions  */
static void probabilisticMutation(struct parameters *params, struct chromosome *chromo, unsigned int * seed);
static void pointMutation(struct parameters *params, struct chromosome *chromo, unsigned int * seed);
static void probabilisticMutationOnlyActive(struct parameters *params, struct chromosome *chromo, unsigned int * seed);
static void singleMutation(struct parameters *params, struct chromosome *chromo, unsigned int * seed);
...
static void mutateRandomParent(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren, unsigned int * seed);
...
DLL_EXPORT struct parameters *initialiseParameters(const int numInputs, const int numNodes, const int numOutputs, const int arity) {
	...
	// Set BP default values
	params->bp_max_epochs = 500;
	...
	params->fitnessFunction = mse;
	strncpy(params->fitnessFunctionName, "mse", FITNESSFUNCTIONNAMELENGTH);
...
/*
	...
	then the default mse fitness function is used.
*/
...
DLL_EXPORT void setCustomReproductionScheme(struct parameters *params, void (*reproductionScheme)(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren, unsigned int * seed), char const *reproductionSchemeName) {
...
*/
DLL_EXPORT void mutateChromosome(struct parameters *params, struct chromosome *chromo, unsigned int * seed) {

	params->mutationType(params, chromo, seed);
...
/*
	...
	loops through parents and then the children so the children is always preferred over the parents
*/
static void getBestChromosome(struct chromosome **parents, struct chromosome **children, int numParents, int numChildren, struct chromosome *best) 
{
	int i;
...
DLL_EXPORT struct chromosome* runCGPBP_IN(struct parameters *params, struct dataSet *dataTrain, struct dataSet *dataValid, struct fann_train_data *trainFann, int numGens, unsigned int * seed) 
{
	int i;
	int gen;

	/* bestChromo found using runCGPBP_IN */
	...
	{		
		...
		/* run BP on the best children of the population to evolve weights */
		runBP(params, childrenChromos[best_i], trainFann);
		...
		*/
		...
		params->reproductionScheme(params, parentChromos, childrenChromos, params->mu, params->lambda, seed);
	}
...
DLL_EXPORT struct chromosome* runCGPBP_OUT(struct parameters *params, struct dataSet *dataTrain, struct dataSet *dataValid, struct fann_train_data *trainFann, int numGens, unsigned int * seed) 
{
	int i;
	int gen;

	/* bestChromo found using runCGPBP_OUT */
	struct chromosome *bestChromo;
	...
	/* run BP on the best individual of the population (bestChromo) with respect to the training+validation data to evolve weights */
    runBP(params, bestChromo, trainFann);
...
/*
	...
	Run backpropagation to evolve weights of a given chromo

*/
DLL_EXPORT void runBP(struct parameters *params, struct chromosome *chromo, struct fann_train_data *trainFann)
{

	// Map cgp chromosome structure into fann network structure
	struct params_fann * p = mapCGP_FANN(chromo);

	// Create fann network
	struct fann * bpnet = fann_create_shortcut_array(p->num_layers, p->layers);

	// Set weight connections of bpnet based on the p->weight_matrix
	int i, j;
	for(i = 0; i < p->size_mtx; i++) // rows (origin)
	{
		for(j = 0; j < p->size_mtx; j++) // columns (destiny)
		{
			if( (i < chromo->numInputs + chromo->numNodes) && (j >= chromo->numInputs) )
			{
				float weight = p->weight_matrix[i][j];

				if( i < chromo->numInputs ) // must consider the bias index (last neuron of input layer of bpnet)
				{
					fann_set_weight(bpnet, i, j+1, weight);
				}
				else
				{
					fann_set_weight(bpnet, i+1, j+1, weight);
				}
			}
		}
	}

	// CONFIGURE backpropagation parameters
	unsigned int epochs_between_reports = 0;
	float desired_error = 0.01;
	fann_set_training_algorithm(bpnet, FANN_TRAIN_BATCH);
	fann_set_activation_function_hidden(bpnet, FANN_SIGMOID);
	fann_set_activation_function_output(bpnet, FANN_SIGMOID);

	// RUN backpropagation using adapted FANN library, modifications: 
	// (i) bias-value is zero; 
	// (ii) do not update zero-valued weights (non-existing cgp connection) and one-valued weights (cgp output connection).
	fann_train_on_data(bpnet, trainFann, params->bp_max_epochs, epochs_between_reports, desired_error);
	
	// Map fann network structure into cgp chromosome structure
	mapFANN_CGP(chromo, bpnet, p);

	// Free fann parameters
	freeParamsFann(p);
...
DLL_EXPORT struct params_fann* mapCGP_FANN(struct chromosome *chromo)
{
	int i, j;
	int in = chromo->numInputs;
	int nodes = chromo->numNodes;
	int out = chromo->numOutputs;
	int size_mtx = in + nodes + out;

	// initialize params_fann structure
	struct params_fann *p = (struct params_fann*) malloc(sizeof(struct params_fann));

	// initialize weight_matrix
	p->size_mtx = size_mtx;
	p->weight_matrix = (double**) malloc( size_mtx * sizeof(double*) );

	for(i = 0; i < size_mtx; i++)
	{
		p->weight_matrix[i] = (double*) malloc( size_mtx * sizeof(double) );

		for(j = 0; j < size_mtx; j++)
		{
			p->weight_matrix[i][j] = 0.00;
		}
	}
	...
	for (i = 0; i < chromo->numNodes; i++) // for all the hidden nodes
	{
		for (j = 0; j < getChromosomeNodeArity(chromo, i); j++)  // for the arity of the node
		{
			p->weight_matrix[ chromo->nodes[i]->inputs[j] ][ chromo->numInputs + i ] += chromo->nodes[i]->weights[j];
		}
	}

	for (i = 0; i < chromo->numOutputs; i++) // for all of the outputs
	{
		p->weight_matrix[ chromo->outputNodes[i] ][ in + nodes + i ] = 1.00;
	}

	// GET num_layers and layers
	p->num_layers = chromo->numNodes + 2; // "1" input layer + "numNodes" hidden layers + "1" output layer
	p->layers = (unsigned int *) malloc(p->num_layers * sizeof(unsigned int));
	
	p->layers[0] = chromo->numInputs; // first layer: input nodes
	p->layers[p->num_layers-1] = chromo->numOutputs; // last layer: output nodes
	for(i = 1; i < p->num_layers - 1; i++) // hidden layers: one node per layer
	{
		p->layers[i] = 1;
	}
...
DLL_EXPORT void mapFANN_CGP(struct chromosome *chromo, struct fann * bpnet, struct params_fann * p)
{

	// GET bpnet weights
	int total_conn = fann_get_total_connections(bpnet);
	float * w = (float*) malloc( total_conn * sizeof(float) );
	fann_get_weights(bpnet, w);

	// FIRST STEP: convert from FANN to WEIGHTS_MATRIX
	int n;
	int layer = chromo->numInputs;
	int bias_index = chromo->numInputs; // bias weight index of bpnet: must be ignored during mapping
	int i = 0; // rows (origin)
	int j = chromo->numInputs; // columns (destiny)

	for(n = 0; n < total_conn; n++)
	{
		if(n != bias_index) // regular neuron weight
		{
			p->weight_matrix[i][j] = w[n];
			i++;

			if(i == j || i == chromo->numInputs + chromo->numNodes)
			{	
				i = 0;
				j++;
			}
		}
		else // bias weight: must be ignored
		{
			if(j < chromo->numInputs + chromo->numNodes) // hidden neurons
			{
				layer++;
				bias_index += layer;
			}
			...
			{
				bias_index += (chromo->numInputs + chromo->numNodes + 1);
			}
		}
	}

	// SECOND STEP: convert from WEIGHTS_MATRIX to CGP
	for (i = 0; i < chromo->numNodes; i++) // for all the hidden nodes
	{
		for (j = 0; j < getChromosomeNodeArity(chromo, i); j++)  // for the arity of the node
		{
			chromo->nodes[i]->weights[j] = p->weight_matrix[ chromo->nodes[i]->inputs[j] ][ chromo->numInputs + i ];
		}
	}

	// CORRECTION: deal with multiple connections from the same input node
	int k;
	for (i = 0; i < chromo->numNodes; i++) // for all the hidden nodes
	{
		for (j = 0; j < getChromosomeNodeArity(chromo, i) - 1; j++)  // for the arity of the node
		{
			for(k = j+1; k < getChromosomeNodeArity(chromo, i); k++)
			{
				if(chromo->nodes[i]->inputs[j] == chromo->nodes[i]->inputs[k])
				{
					chromo->nodes[i]->weights[k] = 0.0;
				}
			}
		}
	}
...
DLL_EXPORT void freeParamsFann(struct params_fann *p) 
{
	/* attempt to prevent user double freeing */
	if(p == NULL) 
	{
	...
	int i;
	for(i = 0; i < p->size_mtx; i++)
	{
		free(p->weight_matrix[i]);
	}

	free(p->weight_matrix);
	free(p->layers);	
	free(p);
}

DLL_EXPORT void setBpMaxEpochs(struct parameters *params, unsigned int bp_max_epochs) 
{
	if (bp_max_epochs > 0) {
		params->bp_max_epochs = bp_max_epochs;
	}
	else {
		printf("\nWarning: bp_max_epochs value must have a value of one or greater. bp_max_epochs value left unchanged as '%u'.\n", params->bp_max_epochs);
	}
}
...
static void mutateRandomParent(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren, unsigned int * seed) {
	...
	for (i = 0; i < numChildren; i++) {
		...
		mutateChromosome(params, children[i], seed);
	}
}
...
/*
	Mean Square Error Fitness Function - Default Fitness Function
*/
static double mse(struct parameters *params, struct chromosome *chromo, struct dataSet *data) 
{
	int i, j;
	...
	int chromo_outputs = getNumChromosomeOutputs(chromo);
	...
	for (i = 0 ; i < getNumDataSetSamples(data); i++) 
	{
		...
		for (j = 0; j < chromo_outputs; j++) 
		{
			double diff = getChromosomeOutput(chromo, j) - getDataSetSampleOutput(data, i, j);
			mse += (diff * diff);			
		}

	}

	return mse / (chromo_outputs * getNumDataSetSamples(data));
}
