...
// Created for CGPBP algorithm !!
// Reads fann format data from cgp format data
FANN_EXTERNAL struct fann_train_data * FANN_API fann_read_train_from_cgp_format(int numSamples, int numInputs, int numOutputs, double ** inputData, double ** outputData)
{
	int i, j;

	struct fann_train_data *data;
	data = fann_create_train(numSamples, numInputs, numOutputs);

	for(i = 0; i != numSamples; i++)
	{
		for(j = 0; j != numInputs; j++)
		{
			data->input[i][j] = inputData[i][j];
		}

		for(j = 0; j != numOutputs; j++)
		{
			data->output[i][j] = outputData[i][j];			
		}
	}
